...
exports.postDaily = async (req, res) => {
    ...
    try {
         ...
         */
        let trendsDaily = await googleTrends.dailyTrends({
            trendDate: day ? day : new Date(), // trend date
            geo: geo ? geo : 'US', // REQUIRED US' will target United States or geo: 'FR' will target France.
...
exports.postByRegion = async (req, res) => {
    try {
        ...
        // Replace specials charts
        keyword = keyword ? keyword.replace(/[^\w\s]/gi, '') : null;
        geo = geo ? geo.replace(/[^\w\s]/gi, '').toUpperCase() : null;
        ...
        let trendsReg = await googleTrends.interestByRegion({
            ...
            resolution: resolution ? resolution : 'CITY', // can be COUNTRY, REGION, CITY or DMA
        });
        // Await
        let promResReg = await trendsReg;
        console.log("results: ", promResReg)
        
        return res.send( promResReg );
...
exports.postRealTime = async (req, res) => {
    try {
        ...
        // Await promise response
        let promResRealTime = await trendsRealTime;
        console.log("got ", promResRealTime)
        ...
        if (searchError === 0) {
            ...
            return res.send(promResRealTime);
